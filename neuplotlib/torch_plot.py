...
def _forward_hook(layer_name: str, forward_layers, namelib) -> Callable:
...
def _invoke_net_forward(net: nn.Module, input_tensor: torch.Tensor, forward_layers: List[dict], namelib: Dict[str, int]):
    for name, layer in net.named_children():
        if isinstance(layer, nn.Module):
            layer.register_forward_hook(_forward_hook(name, forward_layers, namelib))
    with torch.no_grad():
        y = net(input_tensor)
        
        
def analyze_net(net: nn.Module, input_tensor: torch.Tensor) -> List[Dict]:
    ...
    _invoke_net_forward(net, input_tensor, forward_layers, namelib)    # forward job
...
def generate(arch: List[str], filename: str):
